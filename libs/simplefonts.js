...
SimpleFontRenderer.prototype =
{
    ...
    preload : function simplefontsPreloadFn()
    {
        ...
        for (var f in fontsList)
        {
            if (fontsList.hasOwnProperty(f))
            {
                for (var i = 0; i < fontSizes.length; i += 1)
                {
                    g.fontManager.load('fonts/' + fontsList[f] + '-' + fontSizes[i] + '.fnt');
                }
            }
        }
    ...
    hasLoaded : function simpleFontRendererHasLoadedFn()
    {
        ...
        if (g.fontManager.getNumPendingFonts() === 0 &&
            g.shaderManager.getNumPendingShaders() === 0)
        {
            if (!this.technique2D)
            {
                var size;
                ...
                for (var f in fontsList)
                {
                    if (fontsList.hasOwnProperty(f))
                    {
                        ...
                        {
                            size = fontSizes[i];
                            fontObjects[size] = g.fontManager.get('fonts/' + fontsList[f] + '-' + size + '.fnt');
                        }
                        fonts[f] = fontObjects;
                    }
                }

                var shader = g.shaderManager.get('shaders/font.cgfx');
...
SimpleFontRenderer.create = function simpleFontRendererCreateFn()
{
    ...
    simpleFontRenderer.defaultFont = g.fontManager.get("");
...
exports.SimpleFontRenderer = SimpleFontRenderer;
module.id = "simplefontrenderer";
